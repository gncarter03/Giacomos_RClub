...
```
_7.3 Exercises_
1. One thing I noticed about the districution of the the variables is that they all are pretty similar. The mean for the z, x, y for each cut are pretty similar. Z is most likely the depth of the diamond. I would say that length is probably y and x is most likely width. 
```{r}
...
```
2. It looked pretty normal. The amount of diamonds worth 10,000 dollars is much lower than it is at around 1000 dollars. One suprising thing is that there is a gap at 1500 or 1000 and I do not know what thats from.  
```{r}
...
nycflights13::flights %>%
  ...
  geom_boxplot(mapping = aes(color = cancelled), bindwidth = 1/4)


```
_7.4.1 Exercises_
1. The NA calues do not go any where in a histogram because they are not numeric. They cannot be represented as they are not a number. On the other hand a bar graph takes categorical data. In a bar graph NA is just another category to graph.  
...
5. A geom_violin() is different because it is a continuous line, much like the freqpoly, but it is curved. The freq violin is more linear with harsh edges. The histogram is different then the violin because the violin takes two differnt axis. I think in general they are showing you different things. The histo is showing you the number of counts. The violin is showing you the number of things at a particular point. And the frequency plot is showing you the differences in values between two variables. 
```{r}
